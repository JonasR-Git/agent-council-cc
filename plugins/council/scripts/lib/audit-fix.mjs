...
const CONTENT_PROTECT = [
  ...
  [/\b(?:CREATE|ALTER|DROP)\s+TABLE\b|\bknex\.schema\b|\.createTable\s*\(|\bMigrationInterface\b|\bsequelize\.define\s*\(/i, "database migration shape"],
  [/(?:^|\n)\s*pipeline\s*\{|(?:^|\n)\s*stages\s*:\s*(?:\n|\[)|\bJenkinsfile\b|\bgitlab-ci\b|(?:^|\n)on\s*:\s*(?:\n|\{)/i, "CI/pipeline shape"],
  [/-----BEGIN\s+(?:RSA |EC |OPENSSH |PGP |DSA )?PRIVATE KEY-----|\baws_secret_access_key\b\s*[:=]|\bapi[_-]?secret\b\s*[:=]\s*["'][A-Za-z0-9/+=_-]{16,}/i, "secret material shape"]
];
...
export async function runAuditFix(cwd, findings, backends = {}, options = {}, deps = {}) {
  ...
  try {
    ...
    // Oracle baseline preflight (docs/enterprise-fix-design.md §6): only gate on the
    // oracle if the tree is green to begin with — otherwise a NEW diagnostic can't be
    // told apart from a pre-existing one and every fix would falsely revert.
    let oracleGate = false;
    if (runOracle && options.oracleGate !== false) {
      const base = await runOracle();
      oracleGate = Boolean(base.ok);
      if (!oracleGate) log(`oracle (${oracleName}) baseline not green — oracle gate disabled for this run`);
    }
    ...
    for (const task of tasks) {
      for (const finding of task.findings) {
        ...
        try {
          ...
          // tests green but drop or flip an exported name. On by default (--no-snapshot
          // -gate opts out). No-op for files with no exports (non-code targets).
          if (options.snapshotGate !== false) {
            const viol = snapshotViolation(source, readFile(task.file));
            if (viol) {
              git.resetHard(snapshot);
          ...
          // diagnostic is reverted before the (slower) test suite runs.
          if (oracleGate) {
