...
| **M6** | **Multi-file structure fixer** (the crux): transform-plan → planned-touched-set enforcement → union-surface (a move is fine; a drop / same-name collision / lost default reverts) → char-test/coverage/suite → per-transform commit, VERIFIED rollback (a failed revert aborts the batch), blast-radius breaker. **Ships PROPOSE-ONLY**: a consolidation is only COMMITTED when it is an explicit `kind: "deterministic-codemod"` clearing every gate — an LLM/uncertain transform is emitted as a proposal, per §9's rule. DEFERRED for the commit path (documented, not wired): the codemod planner, a whole-repo "no specifier still resolves to a victim" scan + importer-resolution check, and the oracle/mutation/content-protection gates. | `audit-multifix.mjs` | ✅ (propose) |
